...
// 색 원본 = backend/server.js 의 COLOR, 매입가 기준 = backend/pricing.js,
// 자세한 설명 = 시트_표시_규칙.md
// 셋 중 하나를 고치면 나머지도 같이 고칠 것.
const LEGEND = [
  { color: '#FFD99E', label: '행 전체', desc: '업체명에 내셔널/내셔날 포함 (채운 칸도 주황)' },
  ...
  { color: '#B8D9FF', label: '매입', desc: '기획가 업체인데 기획가가 없어 상시로 대체' },
...
function HelpLegend() {
  return (
    <details className="help-legend">
      ...
      <p>
        <b>내셔널 · 내셔널지오그래픽</b>만 <b>기획 입고가</b>가 기본입니다.
        <span className="help-legend-note">
          그 외 업체는 상시 입고가 — 이름만 비슷한 신영인터내셔널·HS인터내셔널 등도 상시.
          주황 행 경고는 더 넓어서 이름에 내셔널/내셔날이 들어가면 전부 칠해집니다(가격과 다른 잣대).
        </span>
      </p>
